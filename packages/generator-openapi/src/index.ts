  ...
  for (const serviceSpec of services) {
    ...
    for (const specification of validSpecFiles) {
      ...
      for (const consumer of consumers) {
        ...
        // Only update consumers that already exist in the catalog. A consumer may
        // belong to another domain or be generated by a later generator, so creating
        // it here can put a duplicate placeholder service in the producer's domain.
        const existingConsumer = await getService(consumer.id, consumer.version || 'latest');
        if (!existingConsumer) {
          continue;
        }

        const consumerVersion = consumer.version || existingConsumer.version;
...
const processMessagesForOpenAPISpec = async (
  ...
  for (const operation of operations) {
    const { requestBodiesAndResponses, sidebar, messageName, ...message } = await buildMessage(
      pathToSpec,
      document,
      operation,
